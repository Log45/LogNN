...
// Decoupled weight decay: param -= lr * (m_hat / (sqrt(v_hat) + eps) + wd * param)
class AdamW {
 public:
  explicit AdamW(std::vector<Variable> params, double lr = 1e-3, double weight_decay = 0.01,
  ...
  void step() {
    ...
    for (auto& p : params_) {
      ...
      for (double& x : vhost) x = std::sqrt(x) + eps_;
      Tensor denom = Tensor::from_data(vhat.get_dims(), vhost, vhat.get_device_type(), vhat.get_device_index());
      Tensor adam_term = mhat.elementwise_mult(denom.reciprocal()).mult(lr_);
      Tensor decay_term = p.data().mult(lr_ * wd_);
      p.set_data(p.data().subtract(adam_term).subtract(decay_term));
    }
  }
